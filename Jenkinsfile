pipeline {
    ...
    stages {
        stage('Choose node') {
            steps {
                script {
                    def nodes = ['SecondFlask', 'thirdFlask']
                    def userInput = input(
                        message: 'Please select a node',
                        parameters: [
                            choice(choices: nodes, description: 'Node to run the pipeline on', name: 'NODE', multiSelect: true)
                        ]
                    )
                    env.NODE = userInput.NODE.join(',')
                }
            }
        }
        stage('Run on selected nodes') {
            parallel {
                for (node in env.NODE.split(',')) {
                    stage(node) {
                        agent { label "${node}" }
                        stages {
                            stage('Git get') {
